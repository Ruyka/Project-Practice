...
#include <afxext.h>
#include "..\MFC test\MFC testDoc.h"
#include "..\MFC test\MFC testView.h"
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(CMFCtestView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(CMFCtestDoc, (CObject*) m_pDocument->GetContainer());
